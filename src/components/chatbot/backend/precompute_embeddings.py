...
def extract_portfolio_content():
    ...
    try:
        ...
        for line in content.split('\n'):
            if 'title:' in line and '{' in content.split('\n')[content.split('\n').index(line)-1]:
                in_project = True
                ...
                tags = [tag.strip().strip('\'') for tag in tags_text.strip('[]').split(',')]
                current_project['tags'] = tags
            elif 'points:' in line and in_project:
                current_project['points'] = []
            elif '\'' in line and 'points:' in content.split('\n')[content.split('\n').index(line)-1] and in_project:
    ...
    try:
        ...
        if skills_file.exists():
            ...
            for category in categories:
                ...
                if category_content:
                    # Extract skill names
                    skill_names = re.findall(r"name:\s*'([^']+)'", category_content.group(1))
                    if not skill_names:
                        skill_names = re.findall(r'name:\s*"([^"]+)"', category_content.group(1))
    ...
    portfolio_data.append("Kushagra has demonstrated strong leadership as the Technical Head of IRIS, MIT WPU's premier technical club, where he leads a team of 20+ developers across multiple technical domains, manages the club's website and technical infrastructure, organizes technical workshops and hackathons, and mentors junior members.")
    
    # Add information about publications
    portfolio_data.append("Kushagra co-authored a research paper titled 'Domain-Specific Conversational AI for IRIS MITWPU: From Research Paper to Production' published in IEEE ICoICC 2025. The paper compares RAG and Fine-Tuned Transformer approaches for domain-specific chatbots.")
    
    # Add information about projects
    portfolio_data.append("Kushagra's notable projects include: Adobe Hackathon Finalist (AI-powered PDF analysis platform), Mitsubishi Hackathon Winner (AI-Powered Superannuation Advisor Dashboard), Government Research Project (STIV-based Velocity Estimation for CWPRS), IRIS Club RAG Chatbot, Bosch Hackathon Finalist (Autonomous Traffic Management System), MedTech Diagnostic LLM Pipeline, and more.")
    
    # Add detailed project information
    portfolio_data.append("DocsVerse is a document research chatbot Kushagra developed that combines OCR with RAG to enable intelligent document analysis and question-answering, processing multiple document formats and providing contextually relevant answers.")
    
    portfolio_data.append("SynapTrack is an innovative project for Parkinson's disease detection using EEG analysis, implementing a hybrid deep learning model combining CNN and LSTM architectures to detect subtle patterns in brain activity, achieving 91% accuracy in early-stage detection.")
    
    portfolio_data.append("The Airfield Wildlife Risk Classification project is a real-time bird detection system using computer vision with YOLOv8 for enhancing aviation safety, with a risk assessment algorithm evaluating factors like bird size, flock density, and flight patterns.")
    
    # Add programming languages information
    portfolio_data.append("Kushagra is proficient in multiple programming languages including Python (Advanced), JavaScript/TypeScript (Advanced), Java (Intermediate), C/C++ (Intermediate), SQL (Advanced), HTML/CSS (Advanced), and Bash (Basic).")
    
    # Add database expertise
    portfolio_data.append("Kushagra is experienced with various database technologies including relational databases (MySQL, PostgreSQL, SQLite), NoSQL databases (MongoDB), vector databases (FAISS, ChromaDB), time-series databases (InfluxDB), and in-memory databases (Redis).")
    
    # Add problem-solving approach
    portfolio_data.append("Kushagra approaches problem-solving methodically by analyzing requirements, researching existing solutions, breaking down complex problems, prioritizing tasks, implementing iteratively, testing continuously, documenting the process, and emphasizing scalable solutions.")
    
    # Add learning approach
    portfolio_data.append("Kushagra has a structured approach to learning new technologies by understanding fundamentals, following documentation, building proof-of-concept projects, exploring advanced features, participating in communities, teaching others, and contributing to open-source projects.")
    
    # Add career goals
    portfolio_data.append("Kushagra's career goals include becoming an expert in AI/ML engineering, contributing to cutting-edge research, building products that solve real-world problems, leading technical teams, bridging academic research and industry implementation, and eventually founding a tech startup.")
    
    # Add interests outside technology
    portfolio_data.append("Outside of technology, Kushagra enjoys playing table tennis, reading science fiction, exploring new technologies, mentoring junior students, listening to music, and travelling and exploring new places.")
    
    # Add strengths and areas for improvement
    portfolio_data.append("Kushagra's strengths include technical versatility, strong problem-solving abilities, leadership skills, adaptability to new technologies, and communication skills. He's working to improve balancing perfectionism with pragmatic delivery, delegating tasks more effectively, and enhancing public speaking skills.")
...
if __name__ == "__main__":
    ...
    with open(Path(__file__).parent / "extracted_content.json", "w", encoding="utf-8") as f:
